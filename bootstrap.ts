...
async function doBootstrap() {
    error("could not find tc (a required Rust companion binary)")
    if (ask("Do you want to build tc from source now?") != "yes")
        return
...
async function ensurePermissionsToRun(params?: RunParameters) {
    ...
    if (missing_perms && should_prompt) {
        ...
        info(`  ${colors.bold("run")}        : to run tc, a companion native binary which generates prompt files`)
        info(`  ${colors.bold("read/write")} : to manipulate prompt files generated by tc`)
